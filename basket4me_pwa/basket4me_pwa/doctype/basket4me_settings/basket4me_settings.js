...
frappe.ui.form.on('Basket4Me Settings', {
    setup(frm) {
        // Mode of Payment has NO `company` field on the parent doctype — it's
        // on the `Mode of Payment Account` child table. Filtering by company
        // requires a server method that joins to the child table.
        const mop_query = function(doc, cdt, cdn) {
            var row = locals[cdt][cdn];
            return {
                query: "basket4me_pwa.api.get_modes_of_payment_for_company",
                filters: { company: row.company || "" }
            };
        };

        // Sales Person Details child table — filter by the row's own company
        frm.fields_dict['sales_person_details'].grid.get_field('mode_of_payment').get_query = mop_query;
        ...
        frm.fields_dict['mode_of_payment_details'].grid.get_field('mode_of_payment').get_query = mop_query;
