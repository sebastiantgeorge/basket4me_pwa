#!/usr/bin/env python3
"""
Generate Basket4Me PWA - Van Sales API Reference V4 (Word Document)
"""
...
toc_items = [
    ...
    "11. Receipts / Payments (7 endpoints)",
...
doc.add_paragraph(
    "The Basket4Me PWA API uses token-based authentication as the PRIMARY auth method. "
    "Call the Login endpoint to obtain api_key and api_secret, "
    "then include them in all subsequent requests via the Authorization header. "
    "No Cookie or CSRF token headers are needed -- this is the recommended approach "
    "for PWA and mobile frontends."
)

doc.add_heading("Token Authentication (Primary Method)", level=2)
doc.add_paragraph(
    ...
    "4. Include in ALL subsequent requests as a single Authorization header.\n"
    "5. This method works for both GET and POST requests without any Cookie or CSRF headers."
)
...
run_note.bold = True
p_note.add_run(
    "Token auth (Authorization: token api_key:api_secret) is the PRIMARY and recommended auth method. "
    "It works for both GET and POST requests. No Cookie or CSRF token headers are needed. "
    "This avoids the browser 'Refused to set unsafe header Cookie' error in PWA/frontend apps. "
    "Do NOT use Cookie-based or CSRF-based authentication for this API."
)
...
pl_endpoints = [
    ...
    {"path": API_PREFIX + "get_price_list_items", "method": "GET", "desc": "Get items in a price list with enhanced details"},
...
add_endpoint_detail(
    ...
    "Get items in a specific price list with enhanced details: default UOM, all UOMs with conversion factor & rate, MRP, standard selling price, last customer rate, and available stock qty. Pass customer param to get last_customer_rate for that specific customer.",
    params=[
        ...
        {"name": "customer", "type": "String", "required": "No", "desc": "Customer name -- used to fetch last_customer_rate from SO/SI history for this customer"},
        {"name": "page_number", "type": "Int", "required": "No", "desc": "Page number (default: 1)"},
...
add_endpoint_detail(
    ...
    "Get full details of a specific sales order. Items include enhanced pricing fields: currency, is_free_item, batch_no, uoms array (with conversion_factor and rate per UOM), mrp, standard_selling_price, last_customer_rate, last_customer_price_list, and last_customer_price_list_rate.",
            ...
            "items": [
                {
                    ...
                    "uoms": [
                        {"uom": "Nos", "conversion_factor": 1.0, "rate": 1.25},
                        {"uom": "Case", "conversion_factor": 24.0, "rate": 30.00}
                    ],
                    "mrp": 2.00,
                    "standard_selling_price": 1.50,
                    "last_customer_rate": 1.25,
                    "last_customer_price_list": "Wholesale Price List",
                    "last_customer_price_list_rate": 1.25
                }
            ]
        }
    },
...
add_endpoint_detail(
    ...
    "Create a new sales invoice with items and optional price list. The rate provided by the frontend for each item is respected and used as-is (no server-side price list override). This allows the salesperson to apply negotiated or custom rates from the app.",
    params=[{"name": "params", "type": "Object", "required": "Yes", "desc": "Invoice details with customer, posting_date, due_date, items, price_list. Each item's rate is used as provided by the frontend."}],
    request_body={
        "params": {
            "customer": "CUST-0001",
...
pay_endpoints = [
    ...
    {"path": API_PREFIX + "update_payment_entry", "method": "POST", "desc": "Update a draft payment entry"},
    {"path": API_PREFIX + "submit_payment_entry", "method": "POST", "desc": "Submit a draft payment entry"},
    {"path": API_PREFIX + "cancel_payment_entry", "method": "POST", "desc": "Cancel a payment entry"},
    {"path": API_PREFIX + "get_receipt_list", "method": "GET", "desc": "Get receipt/payment list with filters"},
...
add_endpoint_detail(
    API_PREFIX + "update_payment_entry", "POST",
    "Update a draft payment entry. Only draft (docstatus=0) payment entries can be updated. Allows updating paid_amount, mode_of_payment, posting_date, reference_no, reference_date, remarks, and invoice references.",
    params=[{"name": "params", "type": "Object", "required": "Yes", "desc": "Object with name (required) and fields to update: paid_amount, mode_of_payment, posting_date, reference_no, reference_date, remarks, invoices"}],
    request_body={
        "params": {
            "name": "PE-0032",
            "paid_amount": 600.00,
            ...
            "reference_no": "TXN-2026-5678",
            "reference_date": "2026-03-22",
            "remarks": "Updated payment with bank transfer",
            "invoices": [
                {"reference_doctype": "Sales Invoice", "reference_name": "SINV-0055", "allocated_amount": 600.00}
            ]
        }
    },
    response_body={
...
add_endpoint_detail(
    API_PREFIX + "submit_payment_entry", "POST",
    "Submit a draft payment entry. Only draft (docstatus=0) payment entries can be submitted. Once submitted, the payment is applied to the referenced invoices.",
    params=[{"name": "params", "type": "Object", "required": "Yes", "desc": "Object with name field"}],
...
add_endpoint_detail(
    ...
    "Get paginated list of receipts/payments. Use payment_type to filter by Receipts ('Receive') or Payments ('Pay'). Supports date range filtering with from_date and to_date.",
    params=[
        {"name": "name", "type": "String", "required": "No", "desc": "Filter by receipt name"},
        {"name": "customer", "type": "String", "required": "No", "desc": "Filter by customer"},
        {"name": "status", "type": "String", "required": "No", "desc": "Filter by status"},
        {"name": "payment_type", "type": "String", "required": "No", "desc": "Filter by payment type: 'Receive' for Receipts, 'Pay' for Payments"},
        {"name": "from_date", "type": "String", "required": "No", "desc": "Start date filter (YYYY-MM-DD)"},
        {"name": "to_date", "type": "String", "required": "No", "desc": "End date filter (YYYY-MM-DD)"},
        {"name": "search", "type": "String", "required": "No", "desc": "General search"},
...
add_endpoint_detail(
    ...
    "Get transaction history across Sales Orders, Sales Invoices, and Sales Returns. Filter by item, customer, doctype, and date range. Returns per-line details with summary totals. Sales Returns are identified by is_return=1 on the Sales Invoice (transaction_type='Sales Return'), not by a separate doctype. The selling_price_list field is included for each transaction.",
    params=[
...
run_note2 = p_note2.add_run("Note on Sales Returns: ")
run_note2.bold = True
p_note2.add_run(
    "Sales Returns are Sales Invoices with is_return=1. They appear with doc_type='Sales Invoice' "
    "and transaction_type='Sales Return'. The qty and amount values are negative for returns. "
    "The selling_price_list field is always included to show which price list was used."
)
...
doc.add_paragraph(
    "All authenticated API requests must include the Authorization header with token-based auth. "
    "This is the PRIMARY and only recommended auth method for this API. "
    "The token is obtained from the login endpoint response. "
    "Do NOT use Cookie-based or CSRF-based authentication."
)
...
run_warn = p_warn.add_run("Important: ")
run_warn.bold = True
p_warn.add_run(
    "Only the Authorization header with 'token api_key:api_secret' format is supported. "
    "Cookie and CSRF-based auth are NOT recommended and will cause issues in PWA/mobile apps."
)
...
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Basket4Me_PWA_API_Reference_V4.docx")
